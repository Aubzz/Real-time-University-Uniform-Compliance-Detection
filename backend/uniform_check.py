...
class UniformCheck:
    def __init__(self):
        self.model = YOLO('./models/best.pt')  # Load our trained model
    ...
    def get_video_with_anomaly_detection(self):
        ...
        ret, frame = self.video.read() # reads a frame from the Camera
        if not ret:
        ...
        for result in results:
            boxes = result.boxes
            for box in boxes:
                if box.conf < self.CONFIDENCE_THRESHOLD: # filters out detection with low confidence
                    continue
                ...
                cv2.putText(frame, text, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_tuple, 1) # adds the class label & confidence to the france
